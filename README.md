...
- `autobuild`: 自動高位合成/論理合成用のスクリプト一式
  ...
  - `autobuild_hls_new.rb`: Vitis HLS (Vitis 統合後) 用自動合成スクリプト
...
なお，Vitis HLS 2023.2 以降追加された，Vitis に統合された IDE を使用する場合は，
`autobuild_hls_new.rb` を代わりに使用してください．
この場合，変更すべき定数は，9行目の `VITIS_DIR` から15行目の `BASE_DIR` までです．
